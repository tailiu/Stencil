...
// 	log_txn, _ := transaction.BeginTransaction()
...
// 		transaction.LogChange(undo_action, display_hint, log_txn)
// 	}
...
func genDummyMigrationLogs() {
	...
	for i := 0; i < logEntryNum; i++ {
		undo_action = auxiliary.RandStrSeq(20)
		// display_hint = genSerializedDisplayHint()
		transaction.LogChange(undo_action, log_txn)
	}
...
func main() {
	...
	// 	fmt.Println(err)
	// }
	transaction.CreateTxnLogTable()
	// txn_id, err := transaction.BeginTransaction()
	// transaction.LogChange("999", txn_id)
	// transaction.LogOutcome(txn_id, "COMMIT")
	// if err != nil {
	// 	fmt.Println(err)
	// }
}
