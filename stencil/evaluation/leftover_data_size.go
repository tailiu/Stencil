...
func getLeftoverDataInRowSize(AppDBConn *sql.DB, 
	mCols map[string][]string, table string, pKey int, AppID string) int64 {
...
func getEntireRowInEvaluation(stencilDBConn *sql.DB, 
	migrationID string) []map[string]interface{} {
	
	query := fmt.Sprintf("select * from evaluation where migration_id = '%s'", 
		migrationID)
...
func getLeftoverDataInRowsSize(stencilDBConn *sql.DB, AppDBConn *sql.DB, 
	...
	for _, data1 := range data {
		...
		if _, ok := checkedRow[key]; ok {
			...
			leftoverDataInRowSize += getLeftoverDataInRowSize(AppDBConn, 
				mCols, table, pKey, AppID)
		}
...
func getEntireUnmappedRowSize(stencilDBConn *sql.DB, AppDBConn *sql.DB, 
	migrationID, AppID string) int64 {
	
	conditions := "dst_table = 'n/a'"
	
	data := getTableKeyInLogicalSchemaOfMigrationWithConditions(stencilDBConn, 
		migrationID, "src", conditions)
...
func GetLeftoverDataSize(stencilDBConn *sql.DB, AppDBConn *sql.DB, 
	AppID, migrationID string) int64 {
	
	entireUnmappedRowSize := getEntireUnmappedRowSize(stencilDBConn, AppDBConn, 
		migrationID, AppID)
	
	// log.Println(entireUnmappedRowSize)
	
	leftoverDataInRowSize := getLeftoverDataInRowsSize(stencilDBConn, AppDBConn, 
		AppID, migrationID)
