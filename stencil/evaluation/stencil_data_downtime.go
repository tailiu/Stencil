...
func getDowntime(data DisplayedData, appID string, evalConfig *EvalConfig) time.Duration {
	...
	return data1["updated_at"].(time.Time).Sub(data1["created_at"].(time.Time))
	
}

func getDataDowntimeInStencil(migrationID string, evalConfig *EvalConfig) []time.Duration {
	...
	for _, data := range displayedData {
		dataDowntime = append(dataDowntime, getDowntime(data, appID, evalConfig))
	}
	return dataDowntime
}