...
def getAppSchema(app_name):
    ...
    # db = DB(host="10.224.45.162", user="root", passwd="123")
    db = DB()
    ...
    for fpath in fpaths:
        ...
        for datum in data:
            ...
            if len(t_attrs) > 0:
                ...
                t_vals = "E'%s'" % "',E'".join( [escapeString(datum[attr]) for attr in t_attrs] )

                if "user" in datum.keys():
                    u_attrs = [ x.lower() for x in datum["user"].keys() if x.lower() in schema["user"]]
                    if len(t_attrs) > 0:
                        u_cols = '"%s"' % '","'.join(u_attrs)
                        u_vals = "E'%s'" % "',E'".join( [escapeString(datum["user"][attr]) for attr in u_attrs] )
